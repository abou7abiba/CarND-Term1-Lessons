"""This module represents the solution for sliding window in lesson 8.
...
def find_lane_pixels(binary_warped, nwindows=9, margin=100, minpix=50):
    ...
    for window in range(nwindows):
        ...
        if len(good_inds) > minpix:
            leftx_current = np.int(np.mean(nonzerox[good_inds]))
...
def fit_polynomial(binary_warped, nwindows=9, margin=100, minpix=50):
    ...
    leftx, lefty, rightx, righty, out_img = find_lane_pixels(binary_warped, nwindows=nwindows, margin=margin, minpix=minpix)
    ...
    draw_points = (np.asarray ([left_fitx, ploty]).T).astype(np.int32)   # needs to be int32 and transposed
    cv2.polylines(out_img, [draw_points], False, (255,255,0), thickness=3)  # args: image, points, closed, color 

    # Draw the polynomial of the right line
    draw_points = (np.asarray ([right_fitx, ploty]).T).astype(np.int32)   # needs to be int32 and transposed
    cv2.polylines(out_img, [draw_points], False, (255,255,0), thickness=3)  # args: image, points, closed, color 
